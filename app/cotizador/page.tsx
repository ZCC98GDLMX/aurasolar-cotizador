...
// lógica separada (en /lib a nivel raíz del repo)
import { bomAntaiForPanelsSingleRow, rowSpacingLEtoLEmm } from "../../lib/antai";
...
interface TariffParams01Bim {
  ...
  kwh_basic_block_bim: number;
  kwh_intermediate_block_bim: number;
...
function billCFEBim(
  ...
  switch (tariff) {
    ...
    case "PDBT": {
      if (usePdbtCal) return round2(costPDBTCalibratedMXN(kwhBim));
...
export default function Page() {
  ...
  const [designSunAltDeg, setDesignSunAltDeg] = useState<number>(25);
  const [spacingExtra, setSpacingExtra] = useState<number>(0.1);
  ...
  const [k2AutoSpan, setK2AutoSpan] = useState<boolean>(true);
  const [k2SpanEwM, setK2SpanEwM] = useState<number>(1.2);
  const [k2NsPresetMM, setK2NsPresetMM] = useState<number | undefined>(undefined);

  // ====== (Nuevo) Estados para cableado DC/AC ======
  // Derates (NOM 310-15) y límite de terminal
  const [fTemp, setFTemp] = useState(1.0);
  const [fBundling, setFBundling] = useState(1.0);
  ...
  const [vmpptMin, setVmpptMin] = useState(200);

  // AC — inversor central (monofásico)
  const [iInv, setIInv] = useState(16); // A salida nominal

  // AC — microinversores
  const [nMic, setNMic] = useState(4);
  ...
  const derate = useMemo(
    () => ({ fTemp, fBundling, terminalTempC: termTemp }),
    [fTemp, fBundling, termTemp]
  );

  const dc = useMemo(() => {
    // 1) Corrientes DC según 690-8 (125%)
    const { iSum, iMax } = pvDcMaxCurrentA({
      stringsParalelo: np,
      iscModuloA: isc,
    });
    ...
    const range = suggestStringRange({
      ...
      inverterVmpptMinV: vmpptMin,
      moduleVocStcV: voc,
      moduleVmpStcV: vmp,
      betaVmpPctPerC: betaVmp,
      tCellHotC: tCellHot,
      tAmbientColdC: tMin,
    });
